...
Düzeltme:
  - Karşılaştırma sorularının gold_answer artık pipeline evaluation ile uyumlu:
    "COMPARISON::<entity1>::<entity2>" formatı.
  - Eski "Karşılaştırma: ..." formatı kaldırıldı.
...
# 3-hop eksik tamamlama: Coach → PLACE_OF_BIRTH → City → COUNTRY → Country
if len(questions) - hop3_start < 15:
    paths_3E = run("""
        MATCH (coach:Entity)-[:HEAD_COACH]->(club:Entity)
        WHERE club.entityId IN $ids
        MATCH (coach)-[:PLACE_OF_BIRTH]->(city:Entity)
    ...
    for p in paths_3E:
        ...
        used3E.add(coach)
        add_question(
            text=f'{club} takımının teknik direktörü {coach}\'nin doğduğu {city} şehri hangi ülkededir?',
            path=[club, "HEAD_COACH", coach, "PLACE_OF_BIRTH", city, "COUNTRY", country],
            answer=country,
            difficulty="3-hop",
            domain="football",
            template="Club → HEAD_COACH → Coach → PLACE_OF_BIRTH → COUNTRY"
        )

# 3-hop hâlâ eksikse: Player → Club → HOME_VENUE → Stadium (extra)
if len(questions) - hop3_start < 15:
    paths_3F = run("""
        MATCH (player:Entity)-[:MEMBER_OF_SPORTS_TEAM]->(club:Entity)
        WHERE club.entityId IN $ids
    ...
    for p in paths_3F:
        ...
        used3F.add(player)
        add_question(
            text=f'{club} takımında oynayan {player}\'nin doğduğu şehir ({city}) ile takımının stadyumu ({stadium}) aynı şehirde midir?',
            path=[player, "MEMBER_OF_SPORTS_TEAM", club, "HOME_VENUE", stadium, "←", player, "PLACE_OF_BIRTH", city],
            answer=stadium,
            difficulty="3-hop",
            domain="football",
            template="Player → Club → HOME_VENUE & Player → PLACE_OF_BIRTH (3-hop extra)"
        )
...
for p in stadium_pairs[:3]:
    ...
    add_question(
        ...
        answer=f"COMPARISON::{c1}({s1})::{c2}({s2})",
...
for p in player_pairs[:2]:
    ...
    add_question(
        ...
        answer=f"COMPARISON::{p1}({c1})::{p2}({c2})",
        difficulty="comparison",
        domain="football",
