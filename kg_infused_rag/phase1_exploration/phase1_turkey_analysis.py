...
Düzeltmeler:
    1. driver.close() artık finally bloğunda — hata olsa bile bağlantı kapanır
    2. turkey_candidates boşsa IndexError yerine anlamlı hata mesajı
"""
...
# ── Ana işlem try/finally bloğunda — driver.close() kesinlikle çalışır ────────
try:
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # ✔ DÜZELTME 1: IndexError riski — liste boşsa açıklayıcı hata mesajıyla çık
    if not turkey_candidates:
        print("\n  ⚠ HATA: Türkiye entity bulunamadı.")
        print("  Neo4j'de description alanlarını kontrol edin.")
        sys.exit(1)
...
# ✔ DÜZELTME 2: finally bloğu — hata olsa da olmasa da bağlantı kapanır
finally:
    driver.close()
